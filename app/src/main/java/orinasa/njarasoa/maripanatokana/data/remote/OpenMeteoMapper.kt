...
private fun parseIsoDateTimeFast(iso: String, utcOffsetSeconds: Int): Long {
    try {
        ...
        return cal.timeInMillis - (utcOffsetSeconds * 1000L)
    } catch (e: Exception) {
        return 0L
    }
}

private fun parseIsoDateFast(iso: String, utcOffsetSeconds: Int): Long {
    try {
        val year = iso.substring(0, 4).toInt()
        val month = iso.substring(5, 7).toInt() - 1
        val day = iso.substring(8, 10).toInt()
...
fun deriveAlerts(c: OpenMeteoCurrent, h: OpenMeteoHourly, d: OpenMeteoDaily, utcOffsetSeconds: Int): List<WeatherAlert> {
    ...
    // Pre-parse timestamps once using the fast parser
    val parsedTimes = h.time.map { parseIsoDateTimeFast(it, utcOffsetSeconds) }
...
fun OpenMeteoResponse.toDomain(locationName: String, locationSubtext: String? = null): WeatherData {
    ...
    val parsedHourlyTimes = hourly.time.map { parseIsoDateTimeFast(it, utcOffsetSeconds) }

    val sunriseEpoch = daily.sunrise.firstOrNull()?.let {
        parseIsoDateTimeFast(it, utcOffsetSeconds) / 1000
    } ?: 0L
    val sunsetEpoch = daily.sunset.firstOrNull()?.let {
        parseIsoDateTimeFast(it, utcOffsetSeconds) / 1000
    } ?: 0L

    val dailySunriseMillis = daily.sunrise.map { parseIsoDateTimeFast(it, utcOffsetSeconds) }
    val dailySunsetMillis = daily.sunset.map { parseIsoDateTimeFast(it, utcOffsetSeconds) }
    ...
    val parsedDailyTimes = daily.time.map { parseIsoDateFast(it, utcOffsetSeconds) }
