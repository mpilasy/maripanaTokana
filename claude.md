...
- **Current Conditions:** Collapsible section with full-width cards for High/Low temp, Wind/Wind Gust, and Sunrise/Sunset (horizontal layout within each card), plus paired detail cards for Temperature/Precipitation, Pressure/Humidity (with dew point), and UV Index/Visibility. Detail card rows use fixed height.
- **Forecasts:** Hourly (24h horizontal LazyRow) and weekly (7-day vertical list), both collapsible. Precipitation probability hidden when 0%. Each collapsible section has a share button (visible when expanded) that captures just that section as PNG. Hourly emojis use per-day sunrise/sunset lists (`dailySunrise`/`dailySunset` in millis) so next-day morning hours correctly show daytime icons.
